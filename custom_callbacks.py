...
class MetropolisExchangeTempAdjustmentCallback(BaseExchangeCallback):
    ...
    def adjust_temperatures(self, temp_adj_count):
        ...
        losses_history_by_temp = {k: np.mean(v) for k, v in losses_history_by_temp.items()}  #for now, use a simple mean
    ...
    # def calc_acceptance_ratio_per_t(self):
    #     n_replicas = self.model.n_replicas
    #     hp_name = self.hpname
    #     hp = self.ordered_hyperparams
    #     hp_values = [h[1] for h in hp[hp_name]]
    #     prob_of_swap_history =  [self.exchange_logs[f'loss_{i}'][-self.temp_adj_step:] for i in range(n_replicas)]
    #     prob_of_swap_history_by_temp = {hp_value: [] for hp_value in hp_values}
    #
    #     for r in range(n_replicas):
    #         for step, hp_value in enumerate(self.exchange_logs[r][hp_name]):
    #             losses_history_by_temp[hp_value].append(losses_history[r][step])
