...
from utils import plot_error, log_exchange_data_mh, log_exchange_data_pbt, LogExchangeLossesCallback, MetropolisExchangeOneHPCallback
...
wandb.init(
  ...
  name="test-cifar10-lr-swap-and-no-swap",
  notes="with fixed bug in exchange",
  config={
    "model_name": "lenet5",
...
train_plt, test_plt = plot_error(config.n_replicas, config.batch_size, config.train_data_size, hp[25000]['learning_rate'], history, history_noswap)
